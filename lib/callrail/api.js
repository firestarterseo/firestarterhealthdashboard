...
// Pulls out whatever the customer actually said they need — the "I need windows"
// / "I have 4 windows to install" part — from a form's raw field data. Form field
// labels vary per client/form ("Message*", "I am interested in..", "Project
// details"), so this scans by keyword in the field name rather than an exact
// match, and joins multiple matching fields together if a form has more than one.
function extractInquiryText(formData) {
  if (!formData || typeof formData !== "object") return null;
  const parts = Object.entries(formData)
    .filter(([key]) => /message|interested|inquiry|comment|detail|project|need/i.test(key))
    .map(([, value]) => (typeof value === "string" ? value.trim() : ""))
    .filter(Boolean);
  return parts.length ? parts.join(" — ") : null;
}

// Fetches individual lead-level detail for a date range — used to populate the
// "Recent Leads" section on the account detail page. Unlike fetchDailyFormMetrics
// (which only returns a per-day count for the health dashboard), this keeps each
// submission's contact info and inquiry text, and tags spam rather than dropping
// it outright, so it can still be reviewed/excluded from the UI without needing
// to re-fetch from CallRail.
export async function fetchFormLeadDetails(apiKey, accountId, companyId, startDate, endDate) {
  const forms = await fetchAllPages(
    `https://api.callrail.com/v3/a/${accountId}/form_submissions.json`,
    apiKey,
    "form_submissions",
  ...
  return forms
    ...
    .map((form) => ({
      ...
      customer_name: form.formatted_customer_name || form.customer_name || null,
      customer_email: form.customer_email || null,
      customer_phone_number: form.formatted_customer_phone_number || form.customer_phone_number || null,
      inquiry: extractInquiryText(form.form_data),
      source: form.source || null,
      is_spam: isSpamSubmission(form),
      raw_form_data: form.form_data || null,
    }));
}
