...
// Filters out obviously bot-filled form submissions. Patterns seen in the wild:
// (1) a scraper leaves the literal field-label text behind ("Name*", "Email*"),
// (2) a purely numeric or keyboard-mash name ("099", "Ssaaaassàa" — 4+ repeated
// characters in a row is not something a real name does), or (3) a bot filling
// garbage into every field (single-letter name + invalid email + all-zeros
// phone). Patterns 1 and 2 are treated as spam on their own since real names
// essentially never look like that; pattern 3 requires at least two red flags
// together, so a real person who just skips the phone field isn't excluded.
function isSpamSubmission(form) {
  ...
  const phoneIsAllZeros = phone.length > 0 && /^0+$/.test(phone);

  if (isPlaceholderName || isNumericName || isGibberishName) return true;
  if (isSingleLetterName && (emailIsInvalid || phoneIsAllZeros)) return true;
  if (emailIsInvalid && phoneIsAllZeros) return true;
  return false;
}
