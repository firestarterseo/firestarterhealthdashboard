...
export function periodStats(rows, defKey, aggType) {
  const col = METRIC_DB_COLUMN[defKey];
  const len = rows.length;
  const toNums = (arr) => arr.map((r) => Number(r[col]) || 0);

  const last30 = agg(toNums(rows.slice(Math.max(0, len - 30))), aggType);
  const prev30 = agg(toNums(rows.slice(Math.max(0, len - 60), Math.max(0, len - 30))), aggType);
  const hasBaseline = len >= 60;
  const first30 = hasBaseline ? agg(toNums(rows.slice(0, 30)), aggType) : null;

  return { last30, prev30, first30, hasBaseline };
}
...
export function buildCompareRows(rows, hasAds) {
  return METRIC_DEFS.filter((d) => !d.optional || hasAds).map((def) => {
    const stats = periodStats(rows, def.key, def.agg);
    const vsPrev = pctChange(stats.last30, stats.prev30);
    const vsStart = stats.hasBaseline ? pctChange(stats.last30, stats.first30) : null;
